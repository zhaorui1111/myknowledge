...
// Icons rendered inline as SVG in highlight output (no Vue components needed for code blocks)
...
let headingIndex = 0 // ensures unique heading ids even if text repeats
...
const md: MarkdownIt = new MarkdownIt({
  ...
  highlight(str: string, lang: string): string {
    ...
    // Wrap in a custom container with language label + copy button slot
    return (
...
// Override heading_open to add id + anchor link
md.renderer.rules.heading_open = (tokens, idx, options, _env, self) => {
  const token = tokens[idx]
  // Get inline text from next token
...
// After each heading_open, inject an anchor link
...
function initScrollSpy() {
  ...
  observer = new IntersectionObserver(
    (entries) => {
      ...
      // Pick the first visible heading (document order)
      ...
      // If none visible, keep last active
    },
    {
...
function scrollToHeading(id: string) {
  ...
  if (el) {
    ...
    activeId.value = id
    // Update URL hash without triggering Vue Router (avoids re-render/flicker)
    history.replaceState(null, "", `#${id}`)
  }
}

/* ─── TOC visibility based on screen width ─── */
...
watch(
  () => route.params.module + "/" + route.params.slug,
  () => {
    loadDoc()
  }
)
...
<template>
  <div class="doc-view">
    ...
    <div v-if="loading" class="doc-view__status">
      <div class="doc-view__spinner"></div>
      <span>加载中...</span>
    </div>

    <div v-if="error" class="doc-view__status doc-view__status--error">
      <span>加载失败: {{ error }}</span>
    </div>

    <!-- Right-side TOC -->
    <aside v-if="showToc && !loading && !error" class="doc-view__toc">
...
/* ─── Code block enhancements (global, not scoped) ─── */
:deep(.code-block) {
  position: relative;
